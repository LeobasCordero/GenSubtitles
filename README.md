...
> **First run:** The default model (`small`) requires a one-time ~470 MB download.
> Use `--model tiny` (~75 MB) for a smaller initial download, or `--model medium` (~1.5 GB) for a larger model.
